...
be rerun and a variety of automated corrections that must be reapplied.

For example, when new SCDB data files were downloaded in May 2021, we had to do the following:

1. Add new SCDB CSV files to `/sources/scdb`:
...
2. Update `/sources/scdb/combine.sh` to create a new `/sources/scdb/decisions.csv`:
...
3. Run `gulp decisions` to create new decisions.json in `/sources/ld` from decisions.csv
4. Run `gulp fixDecisions --dates --overwrite` to fix all decision and argument dates in decisions.json (using `/sources/ld/dates.csv`)

### Manual SCDB Corrections

A variety of "natural court" issues remain in the 2021 data; eg:

- Case 2018-003 should be natural court 1707, not 1706

Next, the "Stone 2" court (1202) was assigned the wrong start date.  Instead of:
...
Stone was promoted to Chief Justice on July 3, 1941 and Byrnes joined on July 8, 1941, so it's reasonable
to combine those two events and set the start of the "Stone 1" court to July 3.  Byrnes left on October 3, 1942
and apparently his seat wasn't filled until Feb 15, 1943 when Rutledge joined.

"Stone 2" extended from Feb 15, 1943 through Oct 1, 1945, when Roberts retired (he actually retired Jul 31, 1945)
...
until June 24, 1946, so the Stone court officially ended on June 23.
...
unlikely Rutledge participated in decisions released on the same day that he joined, but all I've done is mark these cases as
"Stone 2":
...
Another significant problem existed with the "Chase 1" court: numerous "Chase 1" cases reported that JPBradley and WStrong
had voted, but they were not part of that court.  "Chase 1" existed December 15, 1864 through March 13, 1870, whereas
Strong joined Mar 14, 1870 and Bradley joined Mar 23, 1870 (ie, part of "Chase 2").
