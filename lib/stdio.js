...
class stdio
{
    ...
    static sprintf(format, ...args)
    {
        ...
        for (iPart = 0; iPart < aParts.length - 6; iPart += 6) {
            ...
            let modifier = aParts[iPart+4];     // eg, 'h', 'l' or 'L' (all ignored, actually)
            let hash = flags.indexOf('#') >= 0;
            let ach = null, s, radix = 0, p, prefix = "";

            /*
             * The following non-standard sprintf() format codes provide handy alternatives
             * to the PHP date() format codes, also used in datelib.formatDate(), as follows:
             *
