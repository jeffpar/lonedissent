...
class stdio
{
    ...
    static sprintf(format, ...args)
    {
        ...
        for (iPart = 0; iPart < aParts.length - 6; iPart += 6) {
             ...
             *      printf("%C\n", date);
             */
            let ch, date = /** @type {Date} */ (iType < 12 && typeof arg != "object"? datelib.parseDate(arg) : arg);

            switch(type) {
            case 'C':
                ch = hash? '#' : '';
                text += stdio.sprintf(stdio.sprintf("%%%sW, %%%sF %%%sD, %%%sY", ch, ch, ch, ch), date, date, date, date);
                continue;
            ...
            case 'T':
                ch = hash? '#' : '';
                text += stdio.sprintf(stdio.sprintf("%%%sY-%%%s02M-%%%s02D %%%s02H:%%%s02N:%%%s02S", ch, ch, ch, ch, ch, ch), date, date, date, date, date, date);
                continue;
