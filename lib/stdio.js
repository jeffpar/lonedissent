...
class stdio
{
    ...
    static sprintf(format, ...args)
    {
        ...
        for (iPart = 0; iPart < aParts.length - 6; iPart += 6) {
             ...
             */
            let ch, date = /** @type {Date} */ (iType < 12 && typeof arg != "object"? datelib.parseDate(arg) : arg), dateUndefined;

            switch(type) {
            case 'C':
                ch = hash? '#' : '';
                text += (isNaN(date.getTime())? dateUndefined : stdio.sprintf(stdio.sprintf("%%%sW, %%%sF %%%sD, %%%sY", ch), date));
                continue;
            ...
            case 'T':
                ch = hash? '#' : '';
                text += (isNaN(date.getTime())? dateUndefined : stdio.sprintf(stdio.sprintf("%%%sY-%%%s02M-%%%s02D %%%s02H:%%%s02N:%%%s02S", ch), date));
                continue;
